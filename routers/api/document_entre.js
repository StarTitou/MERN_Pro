...
router.post('/',[auth,
    [
    check('ref_document_entr','ref_document_entr is required')
    .not()
    .isEmpty()
]
],async (req,res) =>{
...
try{
    ...
    article = new Document_Entre(Fields);
    await article.save()
    res.json(article);
}catch(err){
    console.error(err.message);
...
router.get('/',async (req,res)=>{
    try {
        const articles = await Document_Entre.find().populate('owner',['name','avatar']);
        res.json(articles);
    } catch (err) {
        console.error(err.message);
        res.status(500).send('Server Error');
