...
router.post('/',[auth,
    [
    check('qte_entre','num_document is required')
    .not()
    .isEmpty()
]
],async (req,res) =>{
...
if(document_entre) Fields.document_entre = document_entre;
...
try{
    let element = await Article_Entre_Magasin.findOne({user:req.user.id});
